...
class ProbEstimator:
    def __init__(self, args):
        ...
        for result_file in glob.glob(
            os.path.join("./fast_detect/local_infer_ref", "*.json")
        ):
    ...
    def crit_to_prob(self, crit):
        offset = np.sort(
            np.abs(np.array(self.real_crits + self.fake_crits) - crit)
        )[100]
        cnt_real = np.sum(
...
def run(args):
    ...
    while True:
        ...
        print(
            f"Fast-DetectGPT criterion is {crit:.4f}, suggesting that the text has a probability of {prob * 100:.0f}% to be fake."
        )
        print()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--scoring_model_name", type=str, default="gpt-neo-2.7B"
    )
